...
use super::{elliptic_curve, Bits, DigestAlgorithm, EllipticCurve, Occurences};
...
mod strict_encoding {
            ...
            fn strict_encode<E: io::Write>(&self, mut e: E) -> Result<usize, Error> {
                Ok(match self {
                    ...
                    Self::Float(bits, min, max) => {
                        let (min, max) = (min.to_le_bytes().to_vec(), max.to_le_bytes().to_vec());
                        let len = (EncodingTag::Float).strict_encode(&mut e)?
                            + bits.strict_encode(&mut e)?;
                        e.write_all(&min)?;
                        e.write_all(&max)?;
                        len + ::core::mem::size_of_val(&min) * 2
    ...
    mod field {
        ...
        impl StrictDecode for Field {
            ...
            fn strict_decode<D: io::Read>(mut d: D) -> Result<Self, Error> {
                Ok(Self(
                    FieldFormat::strict_decode(&mut d)?,
                    Occurences::<u8>::strict_decode(&mut d)?,
                ))
            }
        }
    }
